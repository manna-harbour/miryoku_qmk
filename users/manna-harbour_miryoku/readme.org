...
Firmware uses [handedness by EEPROM](https://docs.qmk.fm/#/feature_split_keyboard?id=handedness-by-eeprom) as default and it must be *configured once* on each side.

Pro-Micro Instructions:
#+BEGIN_SRC sh :tangle no
make ferris/sweep:manna-harbour_miryoku:avrdude-split-left
make ferris/sweep:manna-harbour_miryoku:avrdude-split-right
#+END_SRC

Elite-C or compatible Instructions:
#+BEGIN_SRC sh :tangle no
make ferris/sweep:manna-harbour_miryoku:dfu-split-left
make ferris/sweep:manna-harbour_miryoku:dfu-split-right
#+END_SRC
