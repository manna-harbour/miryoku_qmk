...
The index through pinkie columns home, top, and bottom rows are mapped to the corresponding finger center, north, and south switches, respectively.
The inner index columns home, top, and bottom row keys are mapped to the inner switch on the index, middle, and ring fingers, respectively.
The primary, secondary, and tertiary thumb keys are mapped to the thumb down, nail, and pad switches, respectively.
...
#+BEGIN_SRC sh :tangle no
make voyager:manna-harbour_miryoku:flash # default
make voyager:manna-harbour_miryoku:flash MIRYOKU_MAPPING=SHIFTED_ROWS # shifted rows
#+END_SRC
