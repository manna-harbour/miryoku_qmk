...
Then build with ~manna-harbour_miryoku~ as the keymap name as follows.
...
To select alternative layouts, use the ~-e~ option. E.g.
#+BEGIN_SRC sh :tangle no
...
To select alternative layouts, append to the command line. E.g.
#+BEGIN_SRC sh :tangle no
...
Firmware can be built without a local build environment.  Local tools are still required for [[https://docs.qmk.fm/#/newbs_flashing][flashing]].

First log in to GitHub and fork the [[#miryoku-qmk-development-branch][Miryoku QMK development branch]] repository.
...
The ~alphas~, ~nav~, ~clipboard~, and ~layers~ options correspond to the [[https://github.com/manna-harbour/miryoku/tree/master/docs/reference#alternative-layouts][alternative layout]] options.  Also see the [[../../.github/workflows/test-all-configs.yml][Test All Configs workflow file]] for lists of supported values.  The ~mapping~ option corresponds to the alternative mapping options.  Values for these five options are case-insensitive.

The ~rules~ and ~config~ options can be used to specify values to be appended to ~rules.mk~ and ~config.h~, respectively.  Separate multiple lines with ~\n~.
...
The ~Miryoku Alphas~, ~Miryoku Nav~, ~Miryoku Clipboard~, and ~Miryoku Layers~ options correspond to the [[https://github.com/manna-harbour/miryoku/tree/master/docs/reference#alternative-layouts][alternative layout]] options.  The ~Miryoku Mapping~ option corresponds to the alternative mapping options.  Values for these five options are case-insensitive.

The ~rules.mk~ and ~config.h~ options can be used to specify values to be appended to the corresponding files.  Separate multiple lines with ~\n~.
...
- [[./manna-harbour_miryoku.h]] :: Keymap-related definitions.  Included from ~manna-harbour_miryoku.c~.

Layer data is generated by [[https://github.com/manna-harbour/miryoku_babel][Miryoku Babel]] and is included from files in the [[miryoku_babel]] directory.
